...
def expired_token(jwt_service, valid_user_info):
    ...
    with patch("dm_mcp.domain.auth.services.jwt.datetime") as mock_datetime:
        # 模拟 token 刚创建时的状态
...
class TestJwtServiceCreateToken:
    ...
    def test_create_token_adds_standard_claims(self, jwt_service, valid_user_info):
        ...
        # username 优先级: name > preferred_username > email > sub
        # valid_user_info 中有 email，所以 username 会被设为 email
        assert decoded["username"] == valid_user_info["email"]
...
class TestJwtServiceDecodeToken:
    ...
    def test_decode_token_with_wrong_secret(self, jwt_service):
        """测试使用错误密钥解码"""
        # 用一个服务创建 token
        ...
        wrong_mock.jwt_token_expire_seconds = 3600
        wrong_service = JwtService(
            auth_config_service=wrong_mock,
            app_secret="wrong-secret-key",
        )
...
class TestJwtServiceParseJwtPayload:
    ...
    def test_parse_payload_without_verification(self, jwt_service, valid_user_info):
        ...
        assert payload["username"] == valid_user_info["email"]
    ...
    def test_parse_payload_with_wrong_secret_and_verification(self, jwt_service):
        """测试验证签名时使用错误密钥"""
        token = jwt_service.create_token({"sub": "user123"})
...
class TestJwtServiceFactory:
    ...
    def test_create(self):
        ...
        mock_settings.app_secret.get_secret_value.return_value = (
            "test-secret-for-factory"
        )
        mock_auth_config_service = MagicMock()
        mock_auth_config_service.jwt_token_expire_seconds = 3600

        service = factory.create(mock_settings, auth_config_service=mock_auth_config_service)

        assert isinstance(service, JwtService)
        assert service.app_secret == "test-secret-for-factory"
