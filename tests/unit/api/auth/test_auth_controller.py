...
class TestAuthController:
    ...
    def mock_auth_config_service(self):
        ...
        service.list_providers = AsyncMock(
            return_value=[
                {"name": "github", "enabled": True, "visible": True},
                {"name": "google", "enabled": True, "visible": True},
            ]
        )
        return service

    @pytest.fixture
    def controller(self, mock_settings, mock_oauth_service, mock_auth_config_service):
        """创建OAuth认证控制器"""
        return AuthController(
            settings=mock_settings,
            oauth_services=mock_oauth_service,
    ...
    async def test_handle_oauth_login_with_unsafe_next_url(
        ...
        assert data["error"] == messages.MSG_AUTH_NEXT_URL_INVALID
    ...
    async def test_handle_oauth_providers(self, controller, mock_request):
        ...
        data = json.loads(body)
        assert data["data"] == ["github", "google"]
        controller.auth_config_service.list_providers.assert_called_once()
