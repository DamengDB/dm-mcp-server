...
from dm_mcp.server.auth_backend import AuthBackend
...
class TestAuthBackendInit:
    ...
    def test_init_basic(self):
        ...
        backend = AuthBackend(mock_settings, mock_oauth)

        assert backend.settings is mock_settings
        assert backend.oauth_service is mock_oauth
        assert backend.basic_auth_service is None
    ...
    def test_init_with_all_services(self):
        ...
        mock_settings = MagicMock()
        mock_oauth = MagicMock(spec=OAuthService)
        mock_basic_auth = MagicMock(spec=BasicAuthService)
        ...
        backend = AuthBackend(
            mock_settings,
            mock_oauth,
            basic_auth_service=mock_basic_auth,
            token_service=mock_token_service,
            datasource_service=mock_datasource_service,
        )

        assert backend.basic_auth_service is mock_basic_auth
...
class TestAuthBackendGetTokenAuthContext:
    ...
    async def test_get_token_auth_context_success(self):
        ...
        backend = AuthBackend(
            mock_settings, mock_oauth, token_service=mock_token_service
        )
    ...
    async def test_get_token_auth_context_no_service(self):
        ...
        assert "Token service not available" in str(exc_info.value)
...
class TestAuthBackendAuthenticateMCPRoute:
    ...
    async def test_authenticate_mcp_token_success(self, mock_get_ip):
        ...
        mock_token_service = MagicMock(spec=TokenService)
        mock_token_service.validate_token = AsyncMock(
            return_value=MockTokenConfig(user_id="testuser", datasource_id="ds_123")
        )
        ...
        credentials, user = await backend._authenticate_mcp_token(conn, True)

        assert isinstance(credentials, AuthCredentials)
        assert isinstance(user, MCPUser)

    @pytest.mark.asyncio
    async def test_authenticate_mcp_token_no_header(self):
        """测试 MCP Token 认证无认证头"""
        mock_settings = MagicMock()
        mock_settings.server.base_url = "/api"
        mock_settings.token_auth.enabled = True
        ...
        result = await backend._authenticate_mcp_token(conn, False)
    ...
    async def test_authenticate_mcp_token_wrong_scheme(self):
        ...
        result = await backend._authenticate_mcp_token(conn, True)
...
class TestAuthBackendAuthenticate:
    ...
    async def test_authenticate_bearer_success(self):
        ...
        mock_settings = MagicMock()
        mock_settings.server.base_url = "/api"
        mock_settings.token_auth.enabled = False
...
class TestAuthBackendBasicAuth:
    """_authenticate_basic_auth 方法测试"""

    @pytest.mark.asyncio
    async def test_authenticate_basic_auth_success(self):
        """测试 BasicAuth 认证成功"""
        mock_settings = MagicMock()
        mock_oauth = MagicMock(spec=OAuthService)
        mock_basic_auth = MagicMock(spec=BasicAuthService)
        mock_basic_auth.verify_password = AsyncMock(return_value=True)

        backend = AuthBackend(
            mock_settings, mock_oauth, basic_auth_service=mock_basic_auth
        )

        conn = MagicMock(spec=HTTPConnection)
        conn.headers = Headers({"Authorization": "Basic YWRtaW46cGFzc3dvcmQ="})

        credentials, user = await backend._authenticate_basic_auth(conn, {})

        assert isinstance(credentials, AuthCredentials)
        assert isinstance(user, MCPUser)

    @pytest.mark.asyncio
    async def test_authenticate_basic_auth_no_service(self):
        """测试 BasicAuth 服务不可用"""
        mock_settings = MagicMock()
        mock_oauth = MagicMock(spec=OAuthService)
        ...
        with pytest.raises(AuthenticationError) as exc_info:
            await backend._authenticate_basic_auth(conn, {})

        assert "BasicAuth service not available" in str(exc_info.value)

    @pytest.mark.asyncio
    async def test_authenticate_basic_auth_invalid_format(self):
        """测试 BasicAuth 格式无效"""
        mock_settings = MagicMock()
        mock_oauth = MagicMock(spec=OAuthService)
        mock_basic_auth = MagicMock(spec=BasicAuthService)
        ...
        conn = MagicMock(spec=HTTPConnection)
        conn.headers = Headers({"Authorization": "Basic invalid_format"})

        with pytest.raises(AuthenticationError) as exc_info:
            await backend._authenticate_basic_auth(conn, {})

        assert "Invalid Basic Auth format" in str(exc_info.value)

    @pytest.mark.asyncio
    async def test_authenticate_basic_auth_invalid_username(self):
        """测试 BasicAuth 用户名无效"""
        mock_settings = MagicMock()
        mock_oauth = MagicMock(spec=OAuthService)
        mock_basic_auth = MagicMock(spec=BasicAuthService)
        ...
        conn = MagicMock(spec=HTTPConnection)
        conn.headers = Headers({"Authorization": "Basic dXNlcjpwYXNz"})  # user:pass
    ...
    @pytest.mark.asyncio
    async def test_authenticate_basic_auth_invalid_password(self):
        """测试 BasicAuth 密码无效"""
        mock_settings = MagicMock()
        mock_oauth = MagicMock(spec=OAuthService)
        mock_basic_auth = MagicMock(spec=BasicAuthService)
        mock_basic_auth.verify_password = AsyncMock(return_value=False)
...
class TestAuthBackendMCPTokenIPValidation:
    ...
    async def test_authenticate_mcp_token_generic_error(self):
        ...
        assert "Token authentication failed" in str(exc_info.value)

    @pytest.mark.asyncio
    async def test_authenticate_mcp_token_datasource_disabled(self):
        """测试 MCP Token 绑定数据源已禁用"""
        from dm_mcp.core.exceptions.auth_errors import TokenDatasourceNotFoundError
...
class TestExtractAuthInfoQueryString:
    ...
    def test_extract_auth_info_with_query_string(self):
        ...
        url.path = "/api/test"
        conn.url = url
        conn.query_params = {"query_string": "token=query_token_123"}
        conn.client = MagicMock()
        conn.client.host = "127.0.0.1"
...
class TestAuthenticateMCPRouteViaMain:
    """通过 authenticate 方法测试 MCP 路由分支（第88行）"""

    @pytest.mark.asyncio
    async def test_authenticate_mcp_route_enabled(self):
        """测试 MCP 路由 + Token 认证启用时的分支"""
        mock_settings = MagicMock()
        mock_settings.server.base_url = "/api"
        mock_settings.token_auth.enabled = True
