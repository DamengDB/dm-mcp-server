...
class TestStreamableHttpTransportStart:
    ...
    @patch("dm_mcp.transport.http_transport.uvicorn")
    @patch("dm_mcp.transport.http_transport.LoggingService")
    ...
    def test_start_calculates_workers_from_cpu_count(
        ...
        with patch("multiprocessing.cpu_count", return_value=4):
            with patch.dict("os.environ", {}, clear=False):
                ...
                # 验证 uvicorn.run 被调用，workers 应该被设置
...
class TestStreamableHttpTransportCreateApp:
    ...
    @pytest.mark.skip(reason="需要正确的环境变量设置")
    ...
    @pytest.mark.skip(reason="Test uses unsupported __instancecheck__ magic method")
    @patch("dm_mcp.transport.http_transport.importlib")
    @patch("dm_mcp.transport.http_transport.MCPServer", MagicMock)
...
class TestStreamableHttpTransportSyncSettings:
    ...
    @pytest.mark.skip(reason="Test depends on complex mocking needs review")
    def test_sync_settings_excludes_logging(self, mock_settings):
        """测试同步设置时排除 logging 前缀"""
        pass

    @pytest.mark.skip(reason="Test depends on complex mocking needs review")
    def test_sync_settings_converts_values_to_string(self, mock_settings):
        """测试同步设置时转换值为字符串"""
        pass
