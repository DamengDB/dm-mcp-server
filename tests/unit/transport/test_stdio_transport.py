...
class TestStdioTransport:
    ...
    def test_init_passes_settings_to_parent(self, mock_settings, mock_factory):
        ...
        with patch(
            "dm_mcp.transport.stdio_transport.BaseTransport.__init__"
        ) as mock_parent_init:
            transport = StdioTransport(mock_settings, mock_factory)
    ...
    @pytest.mark.skip(reason="start() calls sys.exit() which is hard to mock properly")
    ...
    @pytest.mark.skip(reason="Mock for async server.run needs more work")
...
class TestStdioTransportStart:
    ...
    async def test_start_normal_flow(self, mock_settings, mock_settings_attrs):
        ...
        with (
            patch(
                "dm_mcp.transport.stdio_transport.stdio_server",
                return_value=mock_streams,
            ),
            patch("dm_mcp.transport.stdio_transport.MCPContext") as mock_mcp_ctx,
            patch("dm_mcp.transport.stdio_transport.uvicorn") as mock_uvicorn,
        ):
    ...
    @pytest.mark.skip(reason="uvicorn mock 复杂，需要更精细的控制")
    @pytest.mark.asyncio
...
class TestStdioTransportInheritance:
    ...
    def test_inherits_from_base_transport(self, mock_settings_attrs, mock_factory):
        """测试继承自 BaseTransport"""
        from dm_mcp.transport import BaseTransport
