...
from dm_mcp.core.db import session
...
class TestInitDb:
    ...
    async def test_init_db_sqlite(self, tmp_path, reset_db_session):
        ...
        with (
            patch("dm_mcp.core.db.session.create_async_engine") as mock_engine,
            patch("dm_mcp.core.db.session.async_sessionmaker") as mock_session_maker,
        ):
    ...
    def test_init_db_skip_if_already_initialized(self, tmp_path, reset_db_session):
        ...
        with patch("dm_mcp.core.db.session.create_async_engine") as mock_engine:
            session.init_db(config)
...
class TestEnsureDamengSchemaIfNeeded:
    ...
    def test_empty_database_skips(self):
        ...
        with patch("dm_mcp.core.db.session.create_engine") as mock_engine:
            session._ensure_dameng_schema_if_needed(cfg)
            mock_engine.assert_not_called()

    @pytest.mark.skip(reason="create_engine mock 复杂，简化测试")
...
class TestEnsureMysqlDatabaseIfNeeded:
    ...
    def test_creates_database_success(self):
        ...
        with (
            patch("dm_mcp.core.db.session.create_engine") as mock_engine,
            patch("dm_mcp.core.db.session.sa_text") as mock_text,
        ):
            mock_conn = MagicMock()
            mock_engine.return_value.__enter__ = MagicMock(return_value=mock_conn)
            mock_engine.return_value.__exit__ = MagicMock(return_value=None)
...
class TestInitDbFull:
    ...
    async def test_init_db_dameng(self, reset_db_session):
        ...
        with (
            patch(
                "dm_mcp.core.db.session._ensure_dameng_schema_if_needed"
            ) as mock_ensure,
            patch("dm_mcp.core.db.session.create_async_engine") as mock_engine,
            patch("dm_mcp.core.db.session.async_sessionmaker") as mock_session_maker,
        ):
    ...
    async def test_init_db_mysql(self, reset_db_session):
        ...
        with (
            patch(
                "dm_mcp.core.db.session._ensure_mysql_database_if_needed"
            ) as mock_ensure,
            patch("dm_mcp.core.db.session.create_async_engine") as mock_engine,
            patch("dm_mcp.core.db.session.async_sessionmaker") as mock_session_maker,
        ):
    ...
    async def test_init_db_postgresql(self, reset_db_session):
        ...
        with (
            patch(
                "dm_mcp.core.db.session._ensure_postgres_database_if_needed"
            ) as mock_ensure,
            patch("dm_mcp.core.db.session.create_async_engine") as mock_engine,
            patch("dm_mcp.core.db.session.async_sessionmaker") as mock_session_maker,
        ):
    ...
    async def test_init_db_dameng_disables_pool_pre_ping(self, reset_db_session):
        ...
        with (
            patch("dm_mcp.core.db.session._ensure_dameng_schema_if_needed"),
            patch("dm_mcp.core.db.session.create_async_engine") as mock_engine,
            patch("dm_mcp.core.db.session.async_sessionmaker") as mock_session_maker,
        ):
            mock_engine_instance = MagicMock()
