...
class TestSettings:
    ...
    def test_default_values(self):
        ...
        sys.argv = [sys.argv[0]]
        try:
            settings = Settings(_env_file=None, app_secret=SecretStr("test-secret"))
    ...
    def test_custom_values(self):
        ...
        try:
            settings = Settings(
                _env_file=None,
                app_secret=SecretStr("test-secret"),
                server=ServerConfig(),
...
class TestSettingsToEnv:
    ...
    def test_to_env_secret_handling(self):
        ...
        try:
            ...
            env = settings.to_env()
            # 验证 SecretStr 被解密
            assert "get_secret_value" not in env.get("APP_SECRET", "")
        finally:
            sys.argv = original_argv
...
class TestSettingsCustomiseSources:
    ...
    def test_cli_args_supported(self):
        ...
        original_argv = sys.argv.copy()
        sys.argv = [sys.argv[0], "--server.port", "9000", "--app_secret", "test"]
        try:
            settings = Settings()
            # 命令行参数应该被解析
