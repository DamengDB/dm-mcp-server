...
import pytest
import uuid
from dm_mcp.infra.persistence import DatasourceContext
...
class TestDatasourceContext:
    ...
    def test_get_without_context(self):
        """测试无上下文时抛出异常"""
        # 需要确保没有设置上下文
        import dm_mcp.infra.persistence.datasource_context as dc_module

        # 保存原始默认值
        orig_default = dc_module._datasource_context_var.get()
        try:
            dc_module._datasource_context_var.set(None)
            with pytest.raises(ValueError, match="未设置数据源上下文"):
