...
class TestMCPServerProvidersLoading:
    ...
    @patch("dm_mcp.app.server.FunctionMCPProvider")
    @patch("dm_mcp.app.server.MCPFunctionRegistry")
    def test_load_mcp_providers(self, mock_registry, mock_function_provider):
        ...
        # Should have Function, Metadata, QueryExec, DpcCluster, Inspection, Data, GenericSql
...
class TestMCPMiddlewaresLoading:
    ...
    @patch("dm_mcp.app.server.MetricsMCPMiddleware")
    @patch("dm_mcp.app.server.TokenAuthMCPMiddleware")
    @patch("dm_mcp.app.server.AuditMCPMiddleware")
...
class TestMCPServerASGIApp:
    ...
    def _make_server(self, tmp_path):
        ...
        docs_dir.mkdir()
        mock_settings.server.static_path = str(static_dir)
        mock_settings.server.docs_path = str(docs_dir)
        mock_settings.server.base_url = ""
        mock_settings.metrics = MagicMock()
        mock_settings.metrics.enabled = False
        mock_settings.metrics.http_path = "/metrics"
    ...
    @patch("dm_mcp.app.server.StreamableHTTPSessionManager")
    def test_create_asgi_app(self, mock_session_manager_cls, tmp_path):
        """测试创建 ASGI 应用"""
        from starlette.applications import Starlette

        mock_session_manager = MagicMock()
        mock_session_manager.run.return_value.__aenter__ = AsyncMock()
        mock_session_manager.run.return_value.__aexit__ = AsyncMock(return_value=None)
        mock_session_manager_cls.return_value = mock_session_manager

        server = self._make_server(tmp_path)
        with patch("dm_mcp.app.server.AuthBackend"):
            app = server.create_asgi_app(stateless=False)

        assert isinstance(app, Starlette)
        mock_session_manager_cls.assert_called_once()
        call_kwargs = mock_session_manager_cls.call_args.kwargs
        assert call_kwargs["stateless"] is False

    @patch("dm_mcp.app.server.StreamableHTTPSessionManager")
    def test_create_asgi_app_stateless(self, mock_session_manager_cls, tmp_path):
        """测试创建无状态 ASGI 应用"""
        mock_session_manager = MagicMock()
        mock_session_manager.run.return_value.__aenter__ = AsyncMock()
        mock_session_manager.run.return_value.__aexit__ = AsyncMock(return_value=None)
        ...
        server = self._make_server(tmp_path)
        with patch("dm_mcp.app.server.AuthBackend"):
            server.create_asgi_app(stateless=True)
...
class TestMCPServerHTTPMiddlewares:
    ...
    def test_get_http_middlewares_returns_list(self):
        ...
        # Mock AuthBackend
        with patch("dm_mcp.app.server.AuthBackend") as mock_auth_backend:
...
class TestMCPServerRun:
    ...
    def _mock_settings(self, transport: str):
        settings = create_mock_settings()
        settings.server.transport = transport
        return settings

    @patch("dm_mcp.infra.transport.stdio_transport.StdioTransport")
    def test_run_stdio_imports_transport(self, mock_stdio_transport):
        ...
        MCPServer.run(factory, settings_cls=lambda: self._mock_settings("stdio"))

        mock_stdio_transport.assert_called_once()
        mock_transport_instance.start.assert_called_once()

    @patch("dm_mcp.infra.transport.http_transport.StreamableHttpTransport")
    def test_run_http_imports_transport(self, mock_http_transport):
        ...
        MCPServer.run(factory, settings_cls=lambda: self._mock_settings("http"))

        mock_http_transport.assert_called_once()
        mock_transport_instance.start.assert_called_once()
    ...
    def test_run_invalid_transport_raises_error(self):
        ...
        with pytest.raises(ValueError, match="未知的传输方式"):
            MCPServer.run(
                lambda: MagicMock(),
                settings_cls=lambda: self._mock_settings("invalid"),
            )
...
class TestMCPServerGenericTypes:
    ...
    def test_server_with_default_types(self):
        """测试默认类型参数"""
        from dm_mcp.app.server import MCPServer, Settings, GlobalContext
    ...
    def test_server_generic_signature(self):
        """测试服务器泛型签名"""
        from dm_mcp.app.server import MCPServer, T_Settings, T_Context
