...
from dm_mcp.services.basic_auth_service import BasicAuthService, ADMIN_USERNAME
...
class TestBasicAuthService:
    ...
    def jwt_config(self):
        """JWT 配置 fixture"""
        return JwtConfig(
            secret=SecretStr("test-secret-key-for-basic-auth-testing"),
    ...
    async def test_is_initialized_false(self, basic_auth_service):
        """测试检查未初始化状态"""
        with patch(
            "dm_mcp.services.basic_auth_service.get_async_session"
        ) as mock_session:
