...
from dm_mcp.infra.persistence import AdminUserModel, get_async_session
...
class BasicAuthService(BaseService):
    ...
    async def init_password(self, password: str) -> None:
        ...
        # 验证密码长度
        if len(password) < 6:
            raise ValueError(messages.MSG_AUTH_PASSWORD_TOO_SHORT)

        # 检查是否已初始化
        if await self.is_initialized():
            raise ValueError(messages.MSG_AUTH_PASSWORD_ALREADY_INITIALIZED)
    ...
    async def change_password(self, old_password: str, new_password: str) -> None:
        ...
        if len(new_password) < 6:
            raise ValueError(messages.MSG_AUTH_NEW_PASSWORD_TOO_SHORT)
        ...
        async with get_async_session() as session:
            ...
            if admin_user is None:
                raise ValueError(messages.MSG_AUTH_ADMIN_NOT_EXISTS)

            # 验证旧密码
            if not pwd_context.verify(old_password, admin_user.password_hash):
                raise ValueError(messages.MSG_AUTH_OLD_PASSWORD_INCORRECT)
