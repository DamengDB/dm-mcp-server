...
class DpcClusterMCPProvider(BaseDataSourceMCPProvider):
    ...
    def _register_routes(self) -> None:
        ...
        async def get_dpc_sp_instances():
            ...
            适用场景：DPC 集群下查看 SP 实例；仅 DPC 部署可用。

            Returns:
                Dict[str, Any]: 含 mode, instances（SP 实例列表）, count。
            """
        ...
        async def get_dpc_instances():
            ...
            Returns:
                Dict[str, Any]: 含 instances（实例列表）, count。
            """
        ...
        async def get_dpc_raft_list():
            ...
            Returns:
                Dict[str, Any]: 含 rafts（RAFT 组列表）, count。
            """
        ...
        async def get_dpc_instance_raft_topology():
            ...
            Returns:
                Dict[str, Any]: 含 topology（实例→RAFT 映射）, count。
            """
        ...
        async def get_dpc_esession_detail():
            ...
            Returns:
                Dict[str, Any]: 含 esessions（ESESSION 列表）, count。
            """
        ...
        async def get_dpc_esession_summary():
            ...
            Returns:
                Dict[str, Any]: 含 summary（站点级聚合）, count。
            """
            return await self._tool_get_dpc_esession_summary()

        @self.mcp.tool(requires_token_auth=True)
        async def get_dpc_stask_threads_by_exec_id(exec_id: int):
            """
            按 exec_id 返回 STASK 线程运行信息。
            适用场景：DPC 执行追踪、线程级排查；仅 DPC 部署可用。

            Args:
                exec_id: 目标执行的唯一标识 ID。

            Returns:
                Dict[str, Any]: 含 exec_id, threads（线程列表）, count。
            """
            return await self._tool_get_dpc_stask_threads_by_exec_id(exec_id=exec_id)

        @self.mcp.tool(requires_token_auth=True)
        async def get_dpc_stask_threads_top(top_n: int = 50):
            """
            ...
            Returns:
                Dict[str, Any]: 含 top_n, threads（排序后列表）, count。
            """
            return await self._tool_get_dpc_stask_threads_top(top_n=top_n)

        @self.mcp.tool(requires_token_auth=True)
        async def get_dpc_sql_node_history_by_exec_id(exec_id: int):
            """
            ...
            Returns:
                Dict[str, Any]: 含 exec_id, nodes（节点历史列表）, count。
            """
            return await self._tool_get_dpc_sql_node_history_by_exec_id(exec_id=exec_id)

        @self.mcp.tool(requires_token_auth=True)
        async def get_dpc_sql_node_top(top_n: int = 50):
            """
            ...
            Returns:
                Dict[str, Any]: 含 top_n, nodes（节点类型列表）, count。
            """
    ...
    async def _get_dpc_source(self) -> str:
        ...
        if datasource.deploy_type != "dmdpc":
            raise ValueError(
                f"current datasource deploy_type={datasource.deploy_type}, expected dmdpc"
            )
