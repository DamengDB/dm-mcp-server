...
from dm_mcp.middlewares import (
    AuditMCPMiddleware,
    MetricsMCPMiddleware,
    TokenAuthMCPMiddleware,
)
from dm_mcp.providers.dpc_cluster_provider import DpcClusterMCPProvider
from dm_mcp.providers.function_provider import FunctionMCPProvider
...
from dm_mcp.server.mcp_registry import MCPFunctionRegistry
from dm_mcp.server.middlewares import ExceptionHandlerMiddleware
from dm_mcp.server.middlewares.audit_http_middleware import AuditHTTPMiddleware
from dm_mcp.server.middlewares.utf8_middleware import UTF8ResponseMiddleware
from dm_mcp.settings.settings import Settings, T_Settings

from .auth_backend import AuthBackend
from .global_context import GlobalContext, T_Context
...
class MCPServer(Generic[T_Settings, T_Context]):
    ...
    def _load_mcp_providers(self) -> None:
        ...
        providers = [
            function_provider,
            MetadataMCPProvider(
                self.context.datasource_service, self.context.pool_service
            ),
            QueryExecMCPProvider(
                self.context.datasource_service,
                self.context.pool_service,
            ),
            PoolOpsMCPProvider(
                self.context.datasource_service,
                self.context.pool_service,
    ...
    def _get_http_middlewares(self):
        ...
        return [
            ...
            Middleware(
                ...
                allow_headers=["Authorization", "Content-Type"],  # 允许的 HTTP 头
            ...
            Middleware(
                ...
                backend=AuthBackend(
                    ...
                    self.context.basic_auth_service,
    ...
    def run(
        ...
        if settings.server.transport == "stdio":
            from dm_mcp.transport.stdio_transport import StdioTransport

            transport = StdioTransport(settings, factory)
        elif settings.server.transport == "http":
            from dm_mcp.transport.http_transport import StreamableHttpTransport

            transport = StreamableHttpTransport(settings, factory)
        else:
            raise ValueError(f"未知的传输方式: {settings.server.transport}")
