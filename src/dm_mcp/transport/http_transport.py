...
from dm_mcp.services import LoggingService
from dm_mcp.settings import Settings
from dm_mcp.transport import BaseTransport, T_ServerFactory
...
class StreamableHttpTransport(BaseTransport):
    ...
    def create_app(cls):
        ...
        if not factory_ref:
            raise RuntimeError(
                "Missing DM_MCP_FACTORY_REF env var. Did you start via StreamableHttpTransport?"
            )
        ...
        try:
            ...
            raise RuntimeError(f"Failed to load server factory '{factory_ref}': {e}")
        ...
        if not isinstance(server_instance, MCPServer):
            raise TypeError(
                f"Factory returned {type(server_instance)}, expected MCPServer"
            )
