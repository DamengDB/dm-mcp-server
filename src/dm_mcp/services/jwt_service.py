...
class JwtService(BaseService):
    ...
    def create_token(
        ...
        # 添加过期时间
        expires_in = expires_in or self.jwt_config.token_expire_seconds
        ...
        # 生成 JWT token
        header = {"alg": "HS256"}
        token = jwt.encode(header, payload, self.jwt_config.secret.get_secret_value())
    ...
    def decode_token(self, token: str) -> Dict[str, Any]:
        ...
        """
        try:
            claims = jwt.decode(token, self.jwt_config.secret.get_secret_value())
            claims.validate()
            # 将 Claims 对象转换为字典
            return dict(claims)
    ...
    def parse_jwt_payload(
        ...
        try:
            ...
            parts = token.split(".")
            if len(parts) < 2:
                raise InvalidTokenError("Invalid JWT format: missing payload")
            ...
            try:
                ...
                raise InvalidTokenError(
                    f"Failed to decode JWT payload: {str(e)}"
                ) from e
            ...
            raise InvalidTokenError(f"Invalid JWT token format: {str(e)}") from e
...
class JwtServiceFactory(ServiceFactory):
    ...
    def create(self, settings, **deps) -> JwtService:
