...
from .token_auth_config import TokenAuthConfig
...
class Settings(BaseSettings):
    ...
    oauth: OAuthConfig = Field(default=OAuthConfig())
    pool: DmPoolConfig = Field(default=DmPoolConfig())
    token_auth: TokenAuthConfig = Field(default=TokenAuthConfig())
    jwt: JwtConfig = Field(default=JwtConfig())
    ...
    def to_env(
        ...
        def _json_encoder(obj):
            ...
            if isinstance(obj, BaseModel):
                # 支持 Pydantic 模型（ DataSourceConfig / DmPoolConfig）
